#include "cmdpwm.h"
#include "psu.h"


void CmdPwm::help(void){
    console->print("\nUsage: pwm <ch> <command> [parameters]\n");
    console->print("PWM control commands\n\n");
    console->print("Channel\t: 1-3\n\n");
    console->print("val\t: set channel value, [value] 0-1024\n");
    console->print("\tControl mode (no value specified on command):\n");
    console->print("\t\t+/-, change duty\n");
    console->print("\t\tv, enter value\n");
    console->print("\t\ts, stop\n\n");
    console->print("cfg <min> <max>\t: Configure pwm range\n\n");
    console->printf("Pins:\n"
                "\tPWM1 (PB4): %d\n"
                "\tPWM2 (PB5): %d\n"
                "\tPWM3 (PB0): %d\n", PWM_Get(0), PWM_Get(1), PWM_Get(2));
}

char CmdPwm::execute(int argc, char **argv){
    char c;
    int32_t ch, val;
    char line[5];

    if(argc == 1){
        help();
        return CMD_BAD_PARAM;
    }

    ia2i(argv[1], &ch);

    if(ch < 1 || ch > 3){
        help();
        return CMD_BAD_PARAM;
    }

    ch--;

    if(!xstrcmp(argv[2], "val")){
        if(ia2i(argv[3], &val)){
            PWM_Set(ch, val);
        }else{
            val = PWM_Get(ch);
            while( (c = console->getChar()) != 'q'){
                switch(c){
                    case '+':
                        if(val < PWM_MAX_VALUE){
                            PWM_Set(ch, ++val);
                        }
                        console->printf("\r%u", val);
                        break;
                    case '-':
                        if(val > PWM_MIN_VALUE){
                            PWM_Set(ch, --val);
                        }
                        console->printf("\r%u", val);
                        break;

                    case 'v':
                        console->printf("\rCH[%u]=", ch + 1);  // restore channel number
                        console->getLine(line, sizeof(line));
                        if(ia2i(line, (int32_t*)&val) == 0){
                            console->printf("\rInvalid value [%d - %d]", PWM_MIN_VALUE, PWM_MAX_VALUE);
                        }else{
                            PWM_Set(ch, val);
                        }
                        break;
                }
            }
            console->printchar('\n');
        }
        return CMD_OK;
    }

    if(xstrcmp(argv[2], "cfg")){
        int32_t min, max;
        if(ia2i(argv[2], &min)){
            if(ia2i(argv[3], &max)){
                psu_setPwmChannelCalibration(ch, min, max);
                return CMD_OK;
            }
        }
        pwmcal_t pwm = psu_getPwmChannelCalibration(ch);
        console->printf("Min: %u, Max %u\n", pwm.min, pwm.max);
        return CMD_OK;
    }

    help();
    return CMD_BAD_PARAM;
}
